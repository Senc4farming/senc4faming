...
import com.example.senc4farming.service.EvalScriptLaunchService;
...
public class AppEvalScriptLaunchController extends AbstractController <EvalScriptLaunchDto> {
    ...
    public String guardarEdicionDatos(@PathVariable("idusr") Integer id, EvalScriptLaunchDto dtoEntrada) {
        ...
        //¿Debería comprobar si hay datos?
        if (dtoControl.isPresent() && evalScript.isPresent() ){
    ...
    public String showFile(@PathVariable("idfile") Integer id,
        ...
        if (evalScriptLaunchDto.isPresent()) {
            ...
            logger.info("Creando el directorio: %s " ,uploadDir );
            //check if user folder exists if not create
            Files.createDirectories(Paths.get(uploadDir));
            logger.info("directorio creado: %s " ,uploadDir );
