 ...
 * 
 */
package com.example.sen4farming.controller;
...
public class AppCalendarController {
	...
	public ModelAndView doPost(@RequestParam String year) {
		...
		setAllHolidays(); // isHoliday field of date set to true if its a holiday
	...
	private Date[][] dateFormatter(ArrayList<Date> month) {
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 7; j++) {
				if (dayPointer < month.size() && month.get(dayPointer).getDow() == j ) {
